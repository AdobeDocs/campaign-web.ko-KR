...
>title="리소스 유효성 검사 제한"
>abstract="유효성 제한 필드는 미러 페이지나 이미지와 같이 업로드된 리소스에 사용됩니다. 이러한 리소스는 제한된 시간 동안 유효합니다. 한도에 도달하면 더 이상 리소스를 사용할 수 없습니다."


다음 **배달 기간** 필드를 사용하면 글로벌 게재 다시 시도 제한을 입력할 수 있습니다. 즉, Adobe Campaign은 시작 날짜부터 메시지를 전송하고 나서 메시지가 오류만 반환하는 경우 유효성 검사 제한에 도달할 때까지 구성 가능한 일반 재시도를 수행합니다.
...
**자원 유효 한도** 는 주로 미러 페이지 및 이미지에 대해 업로드된 리소스에 사용됩니다. 이 페이지의 리소스는 제한된 시간 동안 유효합니다(디스크 공간을 절약하기 위함). 이 제한 후에는 이러한 리소스를 더 이상 사용할 수 없습니다.
...
* **[!UICONTROL 미러 페이지 강제 생성]**: 게재에 미러 페이지에 대한 링크가 삽입되지 않은 경우에도 이 모드를 사용하여 미러 페이지를 생성합니다.
* **[!UICONTROL 미러 페이지를 생성하지 않음]**: 링크가 게재에 있어도 미러 페이지를 생성하지 않도록 하려면 이 모드를 사용합니다.
* **[!UICONTROL 메시지 식별자만 사용하여 액세스할 수 있는 미러 페이지를 생성합니다]**: 전자 메일 콘텐츠에 미러 페이지 링크가 없는 경우 이 옵션을 사용하여 클라이언트 콘솔에서 게재 로그 창의 미러 페이지 컨텐츠에 액세스할 수 있습니다.
