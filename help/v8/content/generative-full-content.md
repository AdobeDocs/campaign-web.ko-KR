...
audience: end-user
title: 생성 콘텐츠
description: Journey Optimizer에서 AI를 사용하여 완전한 콘텐츠 경험을 생성하는 방법을 알아봅니다.
...
# 전체 콘텐츠 생성 {#generative-full-content}
...
Adobe Campaign 웹의 AI를 사용하여 이메일, 랜딩 페이지 및 푸시 알림 채널 전반에서 완전한 콘텐츠 경험을 생성할 수 있습니다. AI는 대상자에게 반향을 불러일으키는 포괄적인 콘텐츠를 만들어 게재의 영향을 최적화하는 데 도움이 됩니다.

## 이메일 및 랜딩 페이지의 경우 {#email-web-channels}

AI를 사용하여 이메일 게재 및 랜딩 페이지에 대한 완전한 콘텐츠 경험을 제작하여 텍스트와 이미지를 모두 생성합니다. 이 강력한 기능은 모든 디지털 터치포인트에서 대상자와 연결하는 매력적인 브랜드 내 콘텐츠를 만드는 데 도움이 됩니다.

### 액세스 및 구성 {#access-configure}

AI로 콘텐츠 만들기를 시작하려면 먼저 게재를 설정하고 콘텐츠 편집기를 열어야 합니다. 아래 단계를 사용하여 작업 공간을 준비하고 콘텐츠 생성 패널에 액세스합니다.
...
1. 필요에 따라 레이아웃을 개인화하고 **[!UICONTROL 콘텐츠 생성]** 메뉴에 액세스합니다.
...
콘텐츠 변형을 생성한 후 결과를 미세 조정하여 정확한 요구 사항을 충족할 수 있습니다. 브랜드 정렬을 검토하고, 색조와 언어를 조정하고, 게재에서 활성화할 콘텐츠를 준비합니다.
...
AI를 사용하여 푸시 알림용 콘텐츠를 생성하려면 먼저 푸시 게재를 설정하고 콘텐츠 편집기를 엽니다. 아래 단계는 게재를 준비하고 콘텐츠 생성 도구에 액세스하는 과정을 안내합니다.
...
1. **[!UICONTROL 텍스트 설정]** 옵션을 사용하여 메시지를 사용자 지정합니다.
   ...
   * **[!UICONTROL 커뮤니케이션 전략]**: 생성된 텍스트에 가장 적합한 커뮤니케이션 스타일을 선택합니다.
   * **[!UICONTROL 언어]**: 생성된 콘텐츠의 언어를 선택합니다.
   * **[!UICONTROL 음색]**: 푸시 알림의 음색이 대상자에게 울려 퍼집니다. AI는 여러분이 유익하거나, 장난스럽거나, 설득력 있게 들리기를 원하든 관계없이 메시지를 그에 따라 조정할 수 있습니다.

     푸시 알림에 대한 ![텍스트 설정 패널](assets/push-genai-3.png){zoomable="yes"}
...
1. **[!UICONTROL 이미지 설정]** 선택:
   ...
   * **[!UICONTROL 컴포지션]**: 이미지의 프레임 내에 요소를 정렬합니다.
     ...
     ![푸시 알림에 대한 이미지 설정](assets/push-genai-4.png){zoomable="yes"}
...
1. 백분율 아이콘을 클릭하여 **[!UICONTROL 브랜드 맞춤 점수]**&#x200B;를 보고 브랜드에 대한 모든 오정렬을 식별합니다.
   ...
   ![브랜드 정렬 점수가 있는 생성된 푸시 알림 변형](assets/push-genai-6.png){zoomable="yes"}
...
AI를 사용하여 전체 이메일 콘텐츠, 텍스트 및 이미지를 생성하는 방법을 알아봅니다.
