...
* [조각](../content/fragments.md)은(는) 테마 사용 모드와 수동 스타일 지정 모드 간에 서로 호환되지 않습니다.

   * 테마를 사용하지 않고 만든 이메일 콘텐츠에서는 테마 조각을 사용할 수 없습니다.

   * 테마 콘텐츠에서 [조각](../content/fragments.md)을 활용하려면 테마를 사용하여 이 조각을 직접 만들어야 합니다. [자세히 알아보기](#leverage-themes-fragment)
...
* HTML에서 만든 콘텐츠를 사용하는 경우 [호환성 모드](existing-content.md)가 되며 이 콘텐츠에 테마를 직접 적용할 수 없습니다.
   ...
   * 테마를 적용하려면 먼저 가져온 콘텐츠 [을(를) 새 템플릿으로 저장](../content/create-email-templates.md#save-as-template)한 다음 이 템플릿을 테마 호환 콘텐츠로 변환해야 합니다. 그런 다음 이 템플릿을 사용하여 이메일 콘텐츠를 만들 수 있습니다. [이 섹션](#theme-convertor)에서 수동 스타일을 사용하여 만든 템플릿을 변환하는 방법을 알아봅니다.
...
1. 시작하려면 새 [콘텐츠 템플릿](../content/create-email-templates.md)을 만드세요.

1. **[!UICONTROL 테마 만들기 또는 편집]** 옵션을 선택하십시오.

   ![테마 만들기 또는 편집 옵션을 보여 주는 콘텐츠 템플릿 편집기](assets/theme-create.png)

1. Adobe 테마를 선택합니다. 이 예제에서는 **[!UICONTROL 기본 테마]**&#x200B;를 선택하고 **[!UICONTROL 만들기]**&#x200B;를 클릭합니다.

   ![기본 테마를 선택한 테마 선택기 및 만들기 단추](assets/theme-select.png)

1. **[!UICONTROL 내 테마]** 탭에서 사용자 지정 템플릿을 선택하고 **[!UICONTROL 편집]**&#x200B;을 클릭하여 업데이트할 수도 있습니다.

   ![편집 강조 표시가 있는 사용자 지정 테마를 나열하는 내 테마 탭](assets/theme-edit.png)

1. **[!UICONTROL 일반 설정]** 탭에서 브랜드에 맞는 특정 이름을 지정하여 테마를 정의합니다. 전자 메일의 기본 뷰포트 너비를 조정할 수 있습니다<!--and also export the current theme for reuse-->.
...
1. 오른쪽의 레일을 사용하여 다양한 탭을 탐색하고 디자인 설정을 업데이트합니다.
   ...
   ![색상, 텍스트, 간격 및 기타 디자인 설정 탭이 있는 테마 편집기 오른쪽 레일](assets/theme-right-pane.png)
...
1. 오른쪽의 다른 탭을 사용하여 이 테마의 각 단추 요소, 구분선, 추가 이미지 서식 및 격자 레이아웃 간격을 별도로 관리할 수 있습니다.
   ...
   ![테마 편집기 오른쪽 레일의 단추 스타일 컨트롤](assets/theme-buttons.png)
...
1. [!DNL Adobe Campaign]에서 [전자 메일 게재를 만들거나](create-email.md) 기존 게재에서 작업하거나 [콘텐츠 템플릿](../content/create-email-templates.md#create-template-from-scratch)을 만들고 [전자 메일 콘텐츠를 편집](get-started-email-designer.md#start-authoring)합니다.

1. 다음 작업 중 하나를 선택할 수 있습니다.

   * 기본 제공 [전자 메일 템플릿](../content/use-email-templates.md)을(를) 선택하여 전자 메일 Designer을 엽니다. 각 템플릿에 해당하는 기본 테마는 자동으로 적용됩니다.

   * [새로운 콘텐츠를 처음부터 디자인하고](create-email-content.md) **[!UICONTROL 테마 사용]**&#x200B;을 선택하여 미리 정의된 스타일 테마로 시작합니다.

     ![테마 사용 및 수동 스타일 옵션을 사용하여 Designer 시작 화면으로 보내기](assets/theme-from-scratch.png)
...
1. 이메일 Designer에서 오른쪽 레일의 **[!UICONTROL 테마]** 단추를 클릭합니다. 기본 테마 또는 템플릿의 테마가 표시됩니다. 이 테마의 두 색상 변형 간을 전환할 수 있습니다.
   ...
   ![활성 테마 및 색 변형을 표시하는 테마 패널이 열려 있는 Designer 캔버스입니다](assets/theme-default-hero.png)

1. 현재 사용 중인 테마 옆의 화살표를 클릭합니다. 사용 가능한 사용자 지정 및 Adobe 테마 목록이 표시됩니다.

   ![테마 드롭다운이 확장되어 Adobe 테마와 내 테마를 표시함](assets/theme-hero-change.png)

1. **[!UICONTROL 내 테마]**&#x200B;를 클릭하고 만든 테마를 선택합니다.

   ![테마 선택기에서 사용자가 만든 테마가 선택된 내 테마 목록](assets/theme-select-custom.png)
...
1. 콘텐츠 템플릿에서 테마를 선택하면 **[!UICONTROL 테마 편집]** 단추를 클릭하여 업데이트할 수 있습니다. [자세히 알아보기](#create-and-edit-themes)
   ...
   ![테마 패널에서 테마 편집 단추를 사용하여 전자 메일 Designer의 콘텐츠 템플릿](assets/theme-edit-in-template.png){width="40%"}
...
1. 여러 색상 변형을 사용하여 테마를 활용하는 경우 주어진 구조 구성 요소에 대해 특정 변형을 선택할 수 있습니다. 이를 통해 전체 콘텐츠에 대해 색상 변형을 정의하고 하나의 특정 구조에 대해 다른 변형을 사용할 수 있습니다.
   ...
   ![스타일 탭에서 특정 테마의 변형 옵션을 사용하여 선택한 구조](assets/theme-structure-variant.png)
...
잠금 해제 스타일 아이콘을 표시하는 선택한 구성 요소의 ![스타일 탭](assets/theme-unlock-style.png){width="90%"}
...
![흰색 배경에 사용자 지정 빨간색 텍스트 색상이 있는 잠금 해제된 텍스트 구성 요소](assets/theme-unlock-style-ex-white.png){width="80%" align="center" zoomable="yes"}

테마를 변경할 경우 <!--the font size is still 14 and -->해당 구성 요소의 글꼴 색상은 여전히 빨간색이지만 새 테마에 따라 이 구성 요소의 배경색이 변경됩니다.

![새 테마에서 배경색과 빨간색 색상이 유지된 잠금 해제된 동일한 텍스트](assets/theme-unlock-style-ex-colored.png){width="80%"}
...
1. [!DNL Adobe Campaign]에서 시각적 조각을 만들고 **[!UICONTROL 만들기]**&#x200B;를 클릭하여 조각의 콘텐츠를 디자인합니다. [방법 알아보기](../content/create-fragment.md#create-from-scratch)

1. 미리 정의된 스타일 테마로 시작하려면 **[!UICONTROL 테마 사용]**&#x200B;을 선택하세요.

   ![선택한 테마를 사용한 조각 전자 메일 Designer 시작 옵션](assets/fragment-use-themes.png){width="100%"}
...
1. 오른쪽 레일에서 **[!UICONTROL 테마]** 단추를 클릭합니다. 기본 테마가 표시됩니다. 이 테마의 다른 색상 변형 간을 전환할 수 있습니다.
   ...
   ![기본 테마를 표시하는 테마 레일이 있는 전자 메일 Designer의 콘텐츠 조각](assets/fragment-default-theme.png){width="100%" align="center" zoomable="yes"}

1. 다른 테마를 선택하여 조각 콘텐츠를 미리 볼 수 있습니다. 이렇게 하려면 기본 테마 옆에 있는 화살표를 선택하고 **[!UICONTROL 테마 선택]**&#x200B;을 클릭합니다.

   ![화살표가 있는 테마 헤더 및 조각 미리 보기를 위한 테마 컨트롤 선택](assets/fragment-select-themes.png){width="40%"}

1. **[!UICONTROL Adobe 테마]**&#x200B;와(과) **[!UICONTROL 내 테마]** 탭 사이를 탐색하고 조각에 대해 호환되는 테마를 최대 5개(두 탭 모두에서) 선택할 수 있습니다.

   ![Adobe 테마 및 내 테마 탭과 호환되는 테마 대화 상자 선택](assets/fragment-select-compatible-themes.png){width=70%}
...
1. **[!UICONTROL 기본 테마]** 옆에 있는 화살표를 다시 선택하십시오. 이제 방금 선택한 여러 테마 사이를 전환하여 각 스타일 렌더링을 미리 볼 수 있습니다.
   ...
   ![테마 패널에서 미리 보기 위해 여러 테마를 선택한 조각 캔버스](assets/fragment-selected-themes.png){width=90%}

1. 테마를 더 추가하거나 선택 항목을 변경하려면 **[!UICONTROL 테마 선택]**&#x200B;을 다시 클릭합니다.

## 템플릿을 테마와 호환되도록 합니다 {#theme-convertor}

[!DNL Adobe Campaign]을(를) 사용하면 수동 스타일을 사용하여 만든 템플릿을 테마 호환 콘텐츠로 변환할 수 있습니다. 이 기능은 테마가 [!DNL Adobe Campaign]에 도입되기 전에 콘텐츠 템플릿을 만들었거나 외부 콘텐츠를 가져오는 경우에 특히 유용합니다.

>[!NOTE]
>
> **전자 메일 템플릿**&#x200B;만 테마와 호환되도록 변환할 수 있습니다. 개별 이메일은 변환할 수 없습니다. 먼저 콘텐츠를 템플릿으로 저장해야 합니다.

1. 이메일 [콘텐츠 템플릿](../content/create-email-templates.md)을(를) 열고 이메일 Designer을 사용하여 콘텐츠를 편집합니다.

1. 오른쪽 레일에서 **[!UICONTROL 테마]** 아이콘을 선택하고 **[!UICONTROL 콘텐츠에서 테마 생성]** 단추를 클릭합니다.

   ![콘텐츠 단추에서 테마 생성이 강조 표시된 테마 패널](assets/generate-theme.png){width=100%}

1. **[!UICONTROL 테마 만들기]** 창이 열립니다. [!DNL Adobe Campaign]은(는) 스타일 요소를 자동으로 감지하여 새 테마로 통합합니다.

   ![템플릿 콘텐츠에서 검색된 스타일을 요약하는 테마 대화 상자를 만듭니다](assets/generate-theme-create-window.png){width=90%}
