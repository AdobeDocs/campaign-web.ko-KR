...
미리 정의된 필터는 나중에 사용할 수 있도록 제작하여 저장되는 사용자 정의 필터입니다. 쿼리 모델러를 사용하는 필터링 작업 중에 바로 가기로 사용할 수 있습니다. 예를 들어 데이터 목록을 필터링하거나 게재 대상을 만들 때 바로 가기를 사용할 수 있습니다.
...
에서 사용자 지정 필터를 저장할 수 있습니다. [쿼리 모델러](../query/query-modeler-overview.md) 나중에 사용할 수 있도록 하려면 다음 작업을 수행하십시오. 다음 단계를 수행하십시오.

1. 쿼리 모델러를 열고 필터링 조건을 정의합니다. 아래 예에서는 마드리드에 거주하고 뉴스레터를 구독한 수신자를 필터링합니다.
...
필터가 사전 정의된 필터 목록에 추가됩니다. 필요한 경우 필터를 즐겨찾기로 저장할 수 있습니다. [이 섹션](#fav-filter)에서 자세히 알아보십시오.
...
대상자를 빌드할 때 사전 정의된 필터를 빠른 액세스를 위한 즐겨찾기로 추가할 수 있습니다. [이 섹션](#fav-filter)에서 자세히 알아보십시오.
