...
>title="미리 정의된 필터 관리"
>abstract="이제 Campaign Web 사용자 인터페이스는 특정 요구 사항에 맞게 미리 정의된 필터를 손쉽게 관리하고 맞춤화할 수 있는 사용자 친화적인 인터페이스를 제공합니다. 한 번 제작하여 나중에 사용할 수 있도록 저장합니다."
...
>title="미리 정의된 필터"
>abstract="Campaign Web 사용자 인터페이스는 특정 요구 사항에 맞게 미리 정의된 필터를 손쉽게 관리하고 맞춤화할 수 있는 사용자 친화적인 인터페이스를 제공합니다. 한 번 제작하여 나중에 사용할 수 있도록 저장합니다."
...
그런 다음 현재 컨텍스트에서 사용할 수 있는 사전 정의된 필터의 전체 목록에 액세스하고 **즐겨 찾는 필터** 섹션에 있는 섹션을 참조하십시오. [이 섹션](#fav-filter)에서 즐겨찾기에 대해 자세히 알아보십시오.

예를 들어 미리 정의된 필터로 대상자를 빌드하려면 다음 단계를 따르십시오.

1. 다음으로 이동 **대상** 왼쪽 메뉴에 있는 항목을 클릭하고 **대상자 만들기** [대상] 목록의 왼쪽 위 모서리에 있는 단추입니다.
1. 대상자 이름을 입력하고 **대상자 만들기** 버튼을 클릭합니다.
1. 다음 항목 선택 **쿼리** 활동을 만든 다음 오른쪽 창에서 **대상자 만들기** 단추를 클릭합니다.

   ![](assets/build-audience-from-filter.png){zoomable=&quot;yes&quot;}

1. 다음에서 **필터 선택 또는 저장** 버튼을 클릭하고 **사용자 정의 필터 선택** 옵션을 선택합니다.
...
사전 정의된 모든 필터는 왼쪽 탐색 메뉴의 전용 항목에 함께 그룹화됩니다.

![](assets/list-of-filters.png){zoomable=&quot;yes&quot;}

이 목록에서 위에 자세히 설명된 대로 새 필터를 만들고 아래 나열된 작업을 수행할 수 있습니다.

* 기존 필터를 편집하고 규칙 및 속성을 변경합니다.
* 사전 정의된 필터를 복제합니다.
* 사전 정의된 필터를 삭제합니다.

대상을 구축할 때 사전 정의된 필터를 빠른 액세스를 위한 즐겨찾기로 추가할 수도 있습니다. [이 섹션](#fav-filter)에서 자세히 알아보십시오.
