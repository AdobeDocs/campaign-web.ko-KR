...
1. 기존 프로필의 데이터를 표시하는 동안 **[!UICONTROL 모드]** 드롭다운 목록에서 **[!UICONTROL 기본 대상에서 대체]**&#x200B;를 선택하여 특정 전자 메일 주소 또는 전화 번호로 증명을 보냅니다.
   ...
   >[!CAUTION]
   >
   >게재에 대해 [대상](../audience/about-recipients.md)을(를) 선택하지 않은 경우 **[!UICONTROL 주 대상에서 대체]** 옵션이 회색으로 표시되고 대체 프로필을 선택할 수 없습니다. 또한 반복 유형 게재의 경우 타겟 대상을 선택하더라도 기본 게재가 템플릿이므로 이 옵션은 여전히 회색으로 표시됩니다.
