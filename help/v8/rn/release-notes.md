...
* **데이터 보강 활동의 조정**: **데이터 보강** 이제 활동을 사용하여 Campaign 데이터베이스 스키마의 데이터를 다른 스키마의 데이터와 조정하거나 파일 로드 활동을 사용하여 업로드한 데이터와 같은 임시 스키마에서 가져온 데이터를 조정할 수 있습니다. 예를 들어 이 옵션을 사용하여 업로드된 파일에 지정된 프로필의 국가를 Campaign 데이터베이스의 전용 테이블에서 사용할 수 있는 국가 중 하나와 조정할 수 있습니다. [자세히 보기](../workflows/activities/enrichment.md)
