...
>abstract="**데이터 로드(RDBMS)** 활동은 **데이터 관리** 활동입니다. 이 활동을 사용하여 클라이언트 콘솔로 전환하지 않고도 외부 관계형 데이터베이스에서 워크플로로 바로 데이터를 로드합니다. 추출된 데이터는 워크플로 전체에서 사용 가능하며 타기팅, 보강 또는 추가 데이터 처리에 사용할 수 있습니다."
...
* **시작** 및 [끝](end.md): 워크플로우의 시작과 끝을 그래픽으로 표시합니다. 이는 선택 사항이며 그 자체로 기능적인 영향을 미치지 않습니다.
