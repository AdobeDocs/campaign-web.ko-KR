...
>abstract="**중복 요소를 식별할 수 있는 필드** 섹션에서 **속성 추가** 버튼을 클릭하여 동일한 값을 통해 중복 항목을 식별할 수 있는 이메일 주소, 이름, 성 등의 필드를 지정할 수 있습니다. 필드 순서는 먼저 처리할 필드를 지정합니다."
...
>title="여집합 생성"
>abstract="중복 요소로 제외된 나머지 집단으로 추가 아웃바운드 전환을 생성할 수 있습니다. 이렇게 하려면 **여집합 생성** 옵션을 토글합니다."

>[!CONTEXTUALHELP]
>id="acw_orchestration_deduplication_settings"
>title="중복 제거 설정"
>abstract="수신 데이터에서 중복 요소를 삭제하려면 아래 필드에서 중복 제거 방법을 정의합니다. 기본적으로 하나의 레코드만 유지됩니다. 표현식 또는 속성을 기반으로 중복 제거 모드를 선택합니다. 기본적으로 중복에서 제외할 레코드는 무작위로 선택됩니다."
...
활동은 여러 인바운드 전환을 지원합니다. 두 개 이상의 인바운드 전환을 사용할 수 있는 경우 활동 속성에서 **가입하도록 설정** 섹션을 사용하여 활동에 연결할 전환을 선택합니다. 그러면 선택한 전환이 워크플로 캔버스에서 **중복 제거**&#x200B;에 연결됩니다.
...
1. **가입하도록 설정** 섹션에서 **중복 제거** 활동의 인바운드 전환으로 연결하려는 이전 활동을 확인하십시오. 그러면 선택한 활동이 워크플로 캔버스에서 **중복 제거**&#x200B;에 연결됩니다. **기본 집합** 필드를 사용하여 참조 인바운드 전환을 정의합니다. 다른 세트의 레코드는 기본 세트에 대해 일치하여 중복을 식별합니다.

   >[!NOTE]
   >
   >이 섹션은 두 개 이상의 인바운드 전환을 사용할 수 있는 경우에만 표시됩니다.

1. **중복 요소를 식별할 수 있는 필드** 섹션에서 **속성 추가** 버튼을 클릭하여 동일한 값을 통해 중복 항목을 식별할 수 있는 이메일 주소, 이름, 성 등의 필드를 지정할 수 있습니다. 필드 순서는 먼저 처리할 필드를 지정합니다. [특성을 선택하고 즐겨찾기에 추가하는 방법을 알아보세요](../../get-started/attributes.md).
