...
1. 인바운드 전환의 데이터를 사용하는 경우 각 레코드에 대한 데이터 및 구독 출처를 지정할 수 있는 **[!UICONTROL 추가 정보]** 섹션이 표시됩니다. 이 섹션을 비워두면 워크플로를 실행할 때 날짜나 원본이 설정되지 않습니다.

   * 인바운드 데이터에 해당 서비스에 대한 프로필 구독일을 나타내는 열이 포함되어 있는 경우 **[!UICONTROL 날짜]** 필드에서 해당 열을 선택할 수 있습니다.
