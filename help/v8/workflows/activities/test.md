...
>title="조건"
>abstract="**테스트** 활동에는 여러 개의 출력 전환이 있을 수 있습니다. 워크플로 실행 중에 각 조건은 그 중 하나가 충족될 때까지 순차적으로 테스트됩니다. 어떤 조건도 충족되지 않으면 워크플로는 **[!UICONTROL 기본 조건]** 경로를 따라 계속됩니다. 활성화된 기본 조건이 없으면 이 시점에서 워크플로우가 중지됩니다."
...
1. 워크플로 실행 중에 각 조건은 그 중 하나가 충족될 때까지 순차적으로 테스트됩니다. 어떤 조건도 충족되지 않으면 워크플로는 **[!UICONTROL 기본 조건]** 경로를 따라 계속됩니다. 활성화된 기본 조건이 없으면 이 시점에서 워크플로우가 중지됩니다.
