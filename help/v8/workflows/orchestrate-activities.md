...
[워크플로우를 만든](create-workflow.md)후에는 워크플로우 메뉴에서든 캠페인 내에서든 다른 작업을 조정할 수 있습니다. 이를 위해 워크플로 다이어그램을 구성할 수 있는 시각적 캔버스가 제공됩니다. 이 다이어그램 내에서 다양한 활동을 추가하고 순차적 순서로 연결할 수 있습니다. 가로 및 세로 스크롤 막대가 캔버스 주위에 표시되므로 보려는 영역으로 직접 드래그하여 큰 워크플로를 탐색할 수 있습니다.
...
활동을 제거하려면 캔버스에서 활동을 선택하고 활동 속성에서 **삭제** 아이콘을 클릭하십시오. 사용 가능한 옵션은 [활동 삭제 및 연결 끊기](#delete)를 참조하십시오.
...
* 캔버스에서 활동을 **삭제**&#x200B;합니다. [이 섹션](#delete-activity)을 참조하십시오.
...
## 활동 삭제 및 연결 해제 {#delete}

### 활동 삭제 {#delete-activity}

활동을 삭제하려면 캔버스에서 활동을 선택하고 활동 속성에서 **삭제** 아이콘을 클릭합니다. 확인 대화 상자가 나타납니다.

* 활동이 다른 활동과 연결되어 있지 않으면 삭제를 확인합니다.

  ![단순 활동 삭제](assets/workflow-delete.png)

* 활동이 하나 이상의 후속 활동과 연결된 경우 이를 처리하는 방법을 선택합니다.

  ![여러 활동 삭제](assets/workflow-delete2.png)

  * **모든 후속 활동 삭제**: 동일한 경로에서 활동 및 해당 활동 뒤에 오는 모든 활동을 제거합니다.
  * **이 활동만 삭제**: 선택한 활동만 제거하고 나머지 경로는 다시 연결합니다. 이 옵션은 활동에 한 명의 후임자가 있는 경우에만 사용할 수 있습니다.
  * **새 분기 삭제 및 만들기**: 선택한 활동을 제거하지만 후속 활동을 유지하며 새로운 개별 분기로 이동합니다.

선택을 확인하려면 **삭제**&#x200B;를 클릭하고 삭제하지 않고 대화 상자를 닫으려면 **취소**&#x200B;를 클릭하십시오.

### 전환 연결 해제 {#disconnect-transition}

두 활동 중 하나를 삭제하지 않고 연결을 끊을 수 있습니다. 연결 해제된 전환 뒤에 배치된 활동은 삭제되지 않고 워크플로우의 새로운 개별 분기로 이동됩니다.

이를 통해 워크플로 다이어그램을 재구성하여 삭제할 활동 및 다시 만들 필요 없이 유지할 활동 그룹을 일시적으로 비울 수 있습니다.

단일 전환 시 이 작업을 수행할 수 있습니다.

1. 연결을 끊을 전환을 선택합니다.

1. 전환 속성에서 **연결 끊기** 아이콘을 클릭합니다.

   ![전환 속성 창에서 연결 끊기 아이콘](assets/workflow-transition.png)

   이 아이콘은 전환이 다운스트림 활동으로 이어지는 경우에만 사용할 수 있습니다. 확인 대화 상자가 나타납니다.

1. 확인하려면 **연결 끊기**&#x200B;를 클릭하고 연결을 끊지 않고 대화 상자를 닫으려면 **취소**&#x200B;를 클릭하십시오.

   ![연결 끊기 확인 대화 상자](assets/workflow-transition2.png)

소스 활동에 여러 개의 아웃바운드 전환이 있는 경우(예: 여러 결과 분기가 있는 **Split** 활동 또는 **Fork** 활동) 해당 활동의 속성 창에서 개별적으로 전환 중 하나를 제거할 수 있습니다.

1. 활동을 선택한 다음 **세그먼트** 섹션에서 제거할 전환을 찾습니다.

1. 해당 전환 옆에 있는 휴지통 아이콘을 클릭합니다. 확인 대화 상자가 나타납니다.

   ![세그먼트 결과 옆에 있는 휴지통 아이콘](assets/workflow-transition3.png)

1. 확인하려면 **제거**&#x200B;를 클릭하고, 아무 것도 제거하지 않고 대화 상자를 닫으려면 **취소**&#x200B;를 클릭하십시오.

   ![전환 확인 대화 상자 제거](assets/workflow-transition4.png)
...
>title="활동 속성"
>abstract="이 섹션에서는 수행할 액션, 최대 기간, 시간대, 머신 선호도 및 비동기 동작을 포함한 작업의 실행 설정을 구성할 수 있습니다."
...
>title="초기화 스크립트"
>abstract="이 섹션에서는 활동이 실패할 경우 어떤 상황이 일어나는지 정의할 수 있습니다. 워크플로 중지, 다음 활동으로 이동 또는 사용자 정의 오류 처리 프로세스 트리거와 같은 액션을 선택할 수 있습니다."
...
>title="초기화 스크립트"
>abstract="이 섹션에서는 활동이 시작될 때 실행되는 JavaScript를 추가할 수 있습니다. 변수를 초기화하거나 매개변수를 설정하거나 해당 활동의 실행에 맞는 데이터를 준비하는 데 사용합니다."
